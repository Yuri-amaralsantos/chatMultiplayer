...
io.on('connection', (socket) => {
  ...
  database.getAllMessages((err, messages) => {
    if (err) {
      ...
      // Send the existing messages to the player
  ...
  socket.on('sendMessage', (message) => {
    ...
    database.insertMessage(socket.id, message, (err, messageId) => {
      if (err) {
        console.error('Error saving message:', err);
      } else {
        // Broadcast the new message to all other players
...
process.on('SIGINT', () => {
  ...
  io.emit('serverShutDown', 'The server is shutting down. Please try again later.');
  io.close(() => {
